...
from client.models import CustomUser as UserModel
...
def get_messages(request):
    # x=
    # print(f'3333333 id  {x} ')

    chats = chatMessages.objects.filter(Q(id__gt=request.GET['last_id']),Q(user_from=request.user.id, user_to=request.GET['chat_id']) | Q(user_from=request.GET['chat_id'], user_to=request.user.id))
    new_msgs = []
...
def send_chat(request):
    ...
    User = get_user_model()
    if request.method == 'POST':
        id1=request.POST['user_from']
        id2=request.POST['user_to']
        msg=request.POST['message']
        
        u_from = UserModel.objects.get(id=id1)
        u_to = UserModel.objects.get(id=id2)
        insert = chatMessages(user_from=u_from,user_to=u_to,message=msg)
